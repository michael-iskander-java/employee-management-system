...
public class ScenarioTwoTest {
	...
	public void testA() throws Exception {
		...
		Date date = new SimpleDateFormat("MM/dd/yyyy").parse("01/30/1987");
		NewEmployeeRequestDto employee = new NewEmployeeRequestDto();
		employee.setName("Mahmoud Marey");
		employee.setContractType(ContractType.PERMANENT);
		employee.setJobTitle("Software Consultant");
	...
	//Update substate of `IN-CHECK` state the employee to `WORK_PERMIT_CHECK_PENDING_VERIFICATION`
	@Test
	...
	//Update substate of `IN-CHECK` state the employee to `WORK_PERMIT_CHECK_FINISHED`
	@Test
	public void testD() throws Exception {
		...
		Assert.assertEquals(result.getBody().getEmployeeStatus(), Status.IN_CHECK);
		Assert.assertEquals(result.getBody().getEmployeeSecurityCheckStatus(), SecurityCheckStatus.SECURITY_CHECK_STARTED);
		Assert.assertEquals(result.getBody().getEmployeeWorkPermitStatus(), WorkPermitStatus.WORK_PERMIT_CHECK_FINISHED);
		
		
	}
	
	
	//Update substate of `IN-CHECK` state the employee to `SECURITY_CHECK_FINISHED` (employee is auto-transitioned to `APPROVED` state)
	@Test
