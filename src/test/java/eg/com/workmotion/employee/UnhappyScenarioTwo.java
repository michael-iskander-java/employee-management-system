...
public class UnhappyScenarioTwo {
	...
	public void testA() throws Exception {
		...
		Date date = new SimpleDateFormat("MM/dd/yyyy").parse("08/05/1975");
		NewEmployeeRequestDto employee = new NewEmployeeRequestDto();
		employee.setName("Ramy Atef");
		employee.setContractType(ContractType.PERMANENT);
		employee.setJobTitle("Senior Software Consultant");
	...
	//Update substate of `IN-CHECK` state the employee to `SECURITY_CHECK_FINISHED`
	@Test
	...
	//Update substate of `IN-CHECK` state the employee to `WORK_PERMIT_CHECK_FINISHED`: ❗️✋transition `WORK_PERMIT_CHECK_STARTED` -> `WORK_PERMIT_CHECK_FINISHED` is not allowed
	@Test
